...
# -*- coding: utf-8 -*-
"""
compare_furniture.py

读取一个 3D-Front 布局 json 文件和一个 furniture_library.json 文件，
对布局中每个家具提取 jid / category / sourceCategoryId，按每行一个写入清单文件，
再以 jid 与 sourceCategoryId 分别与 furniture_library.json 中的 id 做比对，
分别统计两者的对应数量并列出具体对应关系（哪些对得上、哪些对不上）。

仅访问当前文件夹内的文件。用法示例：
    python3 compare_furniture.py \
        --layout 3D_Front_example/0a8d471a-2587-458a-9214-586e003e9cf9.json \
        --library furniture_library.json
"""
...
def load_library_ids(library):
    """读取 furniture_library.json，返回其中所有 id 的集合，并记录重复情况。"""
    if not isinstance(library, list):
        sys.exit("错误：furniture_library.json 顶层应为 list。")
    ids = []
    for item in library:
...
def write_listing(items, id_set, out_path):
    ...
    各纵列含义如下：
      ...
      列2 (jid)                 : 家具实例的唯一标识符，用于和 furniture_library.json 的 id 比对。
      列3 (category)            : 布局文件中该家具的类别名称，部分条目缺失，记为 (missing)。
      列4 (sourceCategoryId)    : 家具的来源分类 id，部分条目缺失，记为 (missing)。
      列5 (jid匹配library)       : 该 jid 是否出现在 furniture_library.json 的 id 集合中（是/否/(缺失)）。
      列6 (sourceCategoryId匹配library): 该 sourceCategoryId 是否出现在 id 集合中（是/否/(缺失)）。
    数据行以制表符 \\t 分隔，便于用表格软件或 pandas 读取。
    """
    header_cols = [
        "序号(index)", "jid", "category", "sourceCategoryId",
        "jid匹配library", "sourceCategoryId匹配library",
    ]
    legend = [
        "# 3D-Front 布局家具清单（每行一个家具，并标注与 furniture_library.json 的匹配结果）",
        "# 列含义：",
        "#   列1 序号(index)                 - 家具在布局文件中的出现顺序编号（从 0001 起）",
        "#   列2 jid                         - 家具实例的唯一标识符，用于和 furniture_library.json 的 id 比对",
        "#   列3 category                    - 布局文件中该家具的类别名称，缺失记为 (missing)",
        "#   列4 sourceCategoryId            - 家具的来源分类 id，缺失记为 (missing)",
        "#   列5 jid匹配library              - 该 jid 是否出现在 furniture_library.json 的 id 中：是 / 否 / (缺失)",
        "#   列6 sourceCategoryId匹配library - 该 sourceCategoryId 是否出现在 id 中：是 / 否 / (缺失)",
        "# 数据行以制表符(TAB)分隔",
    ]
...
def build_stat(field, items, id_set):
    """
    统计某个字段（'jid' 或 'sourceCategoryId'）与 library id 的对应关系。
    返回统计结果字典。
    """
...
def write_report(stats, id_total, id_unique, out_path):
    ...
    lines = []
    lines.append("=" * 70)
    lines.append("furniture_library.json 比对报告")
    lines.append("=" * 70)
    lines.append("比较对象：3D-Front 布局文件中的 jid / sourceCategoryId")
    lines.append("          vs  furniture_library.json 中的 id")
    lines.append("furniture_library 中的 id 总数（含重复）: %d" % id_total)
    lines.append("furniture_library 中的唯一 id 数        : %d" % id_unique)
    lines.append("")
    ...
    for st in stats:
        field = st["field"]
        lines.append("-" * 70)
        lines.append("字段 %s 与 library id 的比对" % field)
        lines.append("-" * 70)
        ...
        lines.append("")
        lines.append("对应上的唯一取值数（在 library 中存在）: %d" % len(st["matched"]))
        lines.append("对应不上的唯一取值数（library 中不存在）: %d" % len(st["unmatched"]))
        lines.append("")
        lines.append("[对应上的取值]")
        if st["matched"]:
            for v in st["matched"]:
...
def main():
    parser = argparse.ArgumentParser(
        description="比对 3D-Front 布局中的 jid / sourceCategoryId 与 furniture_library.json 的 id"
    )
    parser.add_argument(
        "--layout",
    ...
    parser.add_argument(
        "--library",
        default="furniture_library.json",
        help="furniture_library.json 文件地址",
    )
    parser.add_argument(
        "--outdir",
        default="output",
    ...
    id_set, id_total = load_library_ids(library)

    # 准备输出目录与文件名
    os.makedirs(args.outdir, exist_ok=True)
    base = os.path.splitext(os.path.basename(args.layout))[0]
    listing_path = os.path.join(args.outdir, "furniture_listing_%s.txt" % base)
    report_path = os.path.join(args.outdir, "furniture_match_report_%s.txt" % base)

    # 1) 家具清单（每行一个，并标注与 library 的匹配结果）
    ...
    report_text = write_report(stats, id_total, len(id_set), report_path)

    # 3) 控制台摘要
    print("\n" + "=" * 70)
    print("摘要")
    print("=" * 70)
    print("furniture_library 唯一 id 数: %d" % len(id_set))
    for st in stats:
        print("-" * 70)
        print("字段 %s：" % st["field"])
